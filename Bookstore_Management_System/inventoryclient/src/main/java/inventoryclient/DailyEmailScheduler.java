...
public class DailyEmailScheduler {
    ...
    public static void main(String[] args) {
        ...
        }, getInitialDelay(), TimeUnit.DAYS.toMillis(1)); // Schedule for daily execution
    }

    private static long getInitialDelay() {
        Calendar calendar = Calendar.getInstance();
        calendar.set(Calendar.HOUR_OF_DAY, 23); // Set to run at 11 PM
        calendar.set(Calendar.MINUTE, 59);
        calendar.set(Calendar.SECOND, 0);
        calendar.set(Calendar.MILLISECOND, 0);

        long delay = calendar.getTimeInMillis() - System.currentTimeMillis();
        return (delay < 0) ? TimeUnit.DAYS.toMillis(1) + delay : delay; // Handle case if time has already passed today
    }
